...
Each IR also carries a bounded `search_parameters` declaration. The current
walk-forward engine supports one primary `indicator_period` target plus the
protective stop-loss range. Template strategies declare their established primary
indicator; custom AI strategies deterministically select the first indicator used
by the entry rule when the model omits this metadata. The model therefore does not
control arbitrary code paths or parameter expressions.
...
- Custom IR can run rolling walk-forward validation by deriving candidate IRs,
  tuning only the declared indicator period and stop loss, and freezing the chosen
  candidate throughout each out-of-sample window.
...
- Walk-forward currently optimizes one declared indicator period and stop loss at
  a time. Multi-parameter Bayesian search, threshold optimization, and nested
  groups remain future extensions.
